...
@implementation DrivingSchoolCell
...
- (void)deleteBtnClick {
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(clickDrivingSchoolCellDeleteBtn:)])
    {
        [self.delegate clickDrivingSchoolCellDeleteBtn:_index];
    }
...
- (void)setOrderModel:(MyOrderModel *)orderModel
{
    ...
    _detailLab.text = [orderModel.productInfo lastObject];
