...
static HJHttpManager *manger = nil;
...
+ (void)load
{
    ...
    manger = [[HJHttpManager alloc]init];
    //同步网络监控
    reach = [Reachability reachabilityForInternetConnection];
    
    manger.hasNetwork = [self reachable];
    ...
    [afNetworkReachabilityManager setReachabilityStatusChangeBlock:^(AFNetworkReachabilityStatus status) {
        ...
        switch (status) {
            case AFNetworkReachabilityStatusNotReachable:{
                HJLog(@"网络不通..");
                manger.networkStatus = NetworkStatusNotReachable;
                manger.hasNetwork = NO;
                break;
            }
            case AFNetworkReachabilityStatusReachableViaWiFi:{
                HJLog(@"网络通过WIFI连接..");
                manger.networkStatus = NetworkStatusReachableViaWiFi;
                manger.hasNetwork = YES;
                break;
            }
            case AFNetworkReachabilityStatusReachableViaWWAN:{
                HJLog(@"网络通过流量连接..");
                manger.networkStatus = NetworkStatusReachableViaWWAN;
                manger.hasNetwork = YES;
                break;
            }
            case AFNetworkReachabilityStatusUnknown:{
                HJLog(@"网络通过未知网络连接..");
                manger.networkStatus = NetworkStatusUnknown;
                manger.hasNetwork = YES;
                break;
            }
            default:
    ...
    HJLog(@"网络状态:%d",manger.hasNetwork);
...
{
    // 状态
    if (reach.currentReachabilityStatus == NotReachable) return NO;
...
+(void)DeleteRequestWithUrl:(NSString *)urlStr param:(NSDictionary *)dict finish:(HttpSuccessBlock)finishBlock failed:(HttpFailedBlock)failedBlock
{
    ...
    [manager DELETE:urlStr parameters:dict success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        NSData *data = (NSData *)responseObject;
        if (finishBlock) {
        ...
        if (failedBlock) {
            ...
            [error.userInfo setValue:@"请求失败" forKey:NSLocalizedDescriptionKey];
            
            NSError *aError = [NSError errorWithDomain:CustomErrorDomain code:HttpDefultFailed userInfo:error.userInfo];
...
+ (AFSecurityPolicy*)customSecurityPolicy
{
    ...
    /*
     常用MIME Type表
     转自http://www.littledai.com/20110408/112443/791
     3gp video/3gpp
