...
- (void)drawRect:(CGRect)rect
{
    ...
    weekdayArray = [self ishave:weekdayArray];
...
//        [minuteArray addObject:num];
    ...
    [minuteArray addObject:@"00"];
    [minuteArray addObject:@"30"];

    for (int i=UUPICKER_MINDATE; i<UUPICKER_MAXDATE; i++) {
        NSString *num = [NSString stringWithFormat:@"%d",i];
        [yearArray addObject:num];
    }
    weekdayArray = [[NSMutableArray alloc]initWithObjects:@"周一",@"周二",@"周三",@"周四",@"周五",@"周六",@"周日", nil];
    
    if (self.datePickerStyle == UUDateStyle_WeekDay) {
        NSString *weekday = [self weekdayStringFromDate:[NSDate date]];
        for (int i = 0; i<weekdayArray.count; i++) {
            NSString *str = weekdayArray[i];
            if ([str isEqualToString:weekday]) {
                [myPickerView selectRow:i inComponent:0 animated:NO];
                break;
            }
        }
    }
...
{
    if (self.datePickerStyle == UUDateStyle_YearMonthDayHourMinute){
        if (ScreenWidth >320 && isIOS7) {
            
            NSString *str1 = [NSString stringWithFormat:@"%f",(float)90/320*ScreenWidth];
            NSString *str2 = [NSString stringWithFormat:@"%f",(float)135/320*ScreenWidth];
            NSString *str3 = [NSString stringWithFormat:@"%f",(float)180/320*ScreenWidth];
            NSString *str4 = [NSString stringWithFormat:@"%f",(float)225/320*ScreenWidth];
            NSString *str5 = [NSString stringWithFormat:@"%f",(float)280/320*ScreenWidth];
            
            [self creatValuePointXs:@[str1,str2,str3,str4,str5]
                          withNames:@[@"年",@"月",@"日",@"时",@"分"]];
        }else if(isIOS7 && ScreenWidth==320)
        {
    ...
    if (self.datePickerStyle == UUDateStyle_YearMonthDay){
        if (isIOS7 && ScreenWidth>320) {
            NSString *str1 = [NSString stringWithFormat:@"%f",(float)120/320*ScreenWidth];
            NSString *str2 = [NSString stringWithFormat:@"%f",(float)180/320*ScreenWidth];
            NSString *str3 = [NSString stringWithFormat:@"%f",(float)250/320*ScreenWidth];
            [self creatValuePointXs:@[str1,str2,str3]
                      withNames:@[@"年",@"月",@"日"]];
            }else if (isIOS7 && ScreenWidth == 320)
            {
    ...
    if (self.datePickerStyle == UUDateStyle_MonthDayHourMinute){
        if (isIOS7 && ScreenWidth>320) {
            ...
            NSString *str1 = [NSString stringWithFormat:@"%f",(float)100/320*ScreenWidth];
            NSString *str2 = [NSString stringWithFormat:@"%f",(float)150/320*ScreenWidth];
            NSString *str3 = [NSString stringWithFormat:@"%f",(float)210/320*ScreenWidth];
            NSString *str4 = [NSString stringWithFormat:@"%f",(float)265/320*ScreenWidth];

        [self creatValuePointXs:@[str1,str2,str3,str4]
                      withNames:@[@"月",@"日",@"时",@"分"]];
            }else if (isIOS7 && ScreenWidth == 320)
            {
    ...
    if (self.datePickerStyle == UUDateStyle_HourMinute){
        if (isIOS7 && ScreenWidth>320) {
            NSString *str1 = [NSString stringWithFormat:@"%f",(float)130/320*ScreenWidth];
            NSString *str2 = [NSString stringWithFormat:@"%f",(float)225/320*ScreenWidth];
        [self creatValuePointXs:@[str1,str2]
                      withNames:@[@"时",@"分"]];
            }else if(isIOS7 && ScreenWidth == 320)
            {
...
- (UIView *)pickerView:(UIPickerView *)pickerView viewForRow:(NSInteger)row forComponent:(NSInteger)component reusingView:(UIView *)view
{
    ...
    switch (self.datePickerStyle) {
        ...
        case UUDateStyle_WeekDay:{
            if (component==0) {
                title = [weekdayArray objectAtIndex:row];
            }
        }
            break;
        default:
    ...
    if (self.datePickerStyle ==UUDateStyle_WeekDay) {
        ...
        if (self.finishBlock) {
            self.finishBlock(nil,
                             ...
                             nil,
                             nil,
                             [weekdayArray objectAtIndex:weekdayIndex]);
        }
        //代理回调
        [self.delegate uuDatePicker:self
                              ...
                              month:nil
                               ...
                               hour:nil
                             minute:nil
                            weekDay:[weekdayArray objectAtIndex:weekdayIndex]];
        return;
    }
